...
function BreadcrumbHeader() {
    ...
    //  const pathNames  = home.concat(pathname)



    //  console.log("pathNames", pathNames[[pathNames.length-1]]);
    
  return (
    <div className='w-full bg-[#FFF1D2] border-t-2 border-b-2 border-[#F7A600] flex justify-between items-center py-8 px-[5%] mt-5'>

 <div className=' PageTitle'>
    <p className='text-[20px] md:text-[25px] capitalize font-bold tracking-wide'>{home[[home.length-1]]}</p>
     </div>

   {/* breadcrumb */}
   <div className='breadcrumb flex gap-2 '> 

   { home.length > 1 ? home.map((path)=>(
    <Breadcrumb>
      <BreadcrumbList>
        <BreadcrumbItem>
